...
function App() {
	...
	const [toast, setToast] = useState<{ readonly show: false } | {
		readonly show: true;
	...
	const handleScanFilter = (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (state.selectedResults.length > 0) {
			if (!confirm("Changing filter options will clear selected users")) {
				// Force re-render. A bit of a hack but had an issue where the checkbox state was
				// still changing in the UI even when not confirming. So updating the state
				// fixes this by synchronizing the checkboxes with the filter statuses in the
				// state.
				setState({...state});
				return;
			}
		...
		setState({
			...state,
			// Make sure to clear selected results when changing filter options. This is to avoid
			// having users selected in the unfollowed queue but not visible in the UI, which would
			// be confusing.
			selectedResults: [],
			filter: {
				...state.filter,
	...
	useEffect(() => {
		const onBeforeUnload = (e: BeforeUnloadEvent) => {
			// Prompt user if he tries to leave while in the middle of a process (searching /
			// unfollowing / etc.) This is especially good for avoiding accidental tab closing
			// which would result in a frustrating experience.
			if (!isActiveProcess) {
				return;
			}
			...
			while (hasNext) {
				...
				setState(prevState => {
					...
					const newState: State = {
						...prevState,
						// Fix: Changed from Math.floor to Math.round to ensure progress reaches
						// 100% Math.floor would leave progress at 99% when near completion
	...
	return (
		<main id="main" role="main" className="iu">
			<section className="overlay">
				...
				{toast.show && <Toast show={toast.show} message={toast.text}
